...
function M.load(pr_key, filename)
	...
	if not success then
		vim.notify(string.format("Failed to parse %s: %s", filename, data), vim.log.levels.ERROR)
		return nil
	end
...
function M.save(pr_key, filename, data)
	...
	if not success then
		vim.notify(string.format("Failed to encode %s: %s", filename, json), vim.log.levels.ERROR)
		return
	end
